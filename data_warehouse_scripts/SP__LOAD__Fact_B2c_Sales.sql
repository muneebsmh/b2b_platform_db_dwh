...
DROP PROCEDURE IF EXISTS `sp__load_fact_b2c_sales`;

DELIMITER $$
CREATE PROCEDURE `sp__load_fact_b2c_sales`()
BEGIN
	DECLARE err_no INT;
    ...
    SET @job_name := 'LOAD FACT_B2C_SALES';
	...
	SET @step_name := 'Truncating Fact B2C Sales Staging Table';
	...
	DELETE FROM `dwh_b2b_platform`.`stg_fact_b2c_sales`;
	SET @rc := ROW_COUNT();
	...
	SET @step_name := 'Populating Fact B2C Sales Staging Table';
	INSERT INTO `job_details_log` (step_name,job_id,job_name,log_date,step_start_time,step_end_time,status,rows_affected)
	VALUES (@step_name,@job_id,@job_name,CURRENT_DATE,CURRENT_TIMESTAMP,NULL,'Started',NULL);
	SET @step_id := (SELECT MAX(step_id) FROM `job_details_log` WHERE step_name=@step_name);
    
	INSERT INTO `dwh_b2b_platform`.`stg_fact_b2c_sales`
    (
		order_id,
        order_item_id,
        customer_id,
    ...
    SELECT 
        ...
        b.quantity,
        b.total_item_value
	FROM `b2b_platform`.`b2c_orders` a
    INNER JOIN `b2b_platform`.`b2c_order_items` b
	...
	SET @step_name := 'Truncating Fact B2C Sales Table';
	INSERT INTO `job_details_log` (step_name,job_id,job_name,log_date,step_start_time,step_end_time,status,rows_affected)
	VALUES (@step_name,@job_id,@job_name,CURRENT_DATE,CURRENT_TIMESTAMP,NULL,'Started',NULL);
	SET @step_id := (SELECT MAX(step_id) FROM `job_details_log` WHERE step_name=@step_name);

	DELETE FROM `dwh_b2b_platform`.`fact_b2c_sales`;
	SET @rc := ROW_COUNT();
	...
	SET @step_name := 'Populating Fact B2C Sales Table';
	INSERT INTO `job_details_log` (step_name,job_id,job_name,log_date,step_start_time,step_end_time,status,rows_affected)
	VALUES (@step_name,@job_id,@job_name,CURRENT_DATE,CURRENT_TIMESTAMP,NULL,'Started',NULL);
	SET @step_id := (SELECT MAX(step_id) FROM `job_details_log` WHERE step_name=@step_name);
    
	INSERT INTO `dwh_b2b_platform`.`fact_b2c_sales`
	(	
		order_id,
        order_item_id,
        customer_id,
	...
	SELECT 
        ...
        a.quantity,
        a.total_item_value
	FROM `dwh_b2b_platform`.`stg_fact_b2c_sales` a
    LEFT JOIN `dwh_b2b_platform`.`dim_customers` b
    ON a.customer_id=b.document_id
    AND a.datetime_of_order>=b.start_date
    AND a.datetime_of_order<b.end_date
    LEFT JOIN `dwh_b2b_platform`.`dim_company_products` c
    ON a.company_id=c.company_cuit
